...
contract GrantPauseExemptProposalTest is Test {
    ...
    function setUp() public {
        ...
        proposal = new GrantPauseExemptProposal(token, PAUSE_EXEMPT_1, PAUSE_EXEMPT_2, PAUSE_EXEMPT_3);
    }
    ...
    function testGrantPauseExemptRole() public {
        ...
        vm.prank(PAUSE_EXEMPT_1);
        token.pausedTransfer(PAUSE_EXEMPT_2, transferAmount * 2);

        vm.prank(PAUSE_EXEMPT_1);
        token.pausedTransfer(PAUSE_EXEMPT_3, transferAmount * 3);

        // Now each transfers to policy
        vm.prank(PAUSE_EXEMPT_1);
        bool success1 = token.pausedTransfer(POLICY_ADDRESS, transferAmount);
        assertTrue(success1, "Transfer 1 token from PAUSE_EXEMPT_1 should succeed");

        vm.prank(PAUSE_EXEMPT_2);
        bool success2 = token.pausedTransfer(POLICY_ADDRESS, transferAmount * 2);
        assertTrue(success2, "Transfer 2 tokens from PAUSE_EXEMPT_2 should succeed");

        vm.prank(PAUSE_EXEMPT_3);
        bool success3 = token.pausedTransfer(POLICY_ADDRESS, transferAmount * 3);
